...
def extract_input_value(controller1_state, controller2_state, remap_key):
    if ':' in remap_key:
        controller, input_key = remap_key.split(':', 1)
        return (controller1_state if controller == "controller1" else controller2_state).get(input_key, 0.0)
    return controller1_state.get(remap_key, controller2_state.get(remap_key, 0.0))
...
async def poll_controller_inputs(controller1, controller2):
    return (
        controller1.get_controller_inputs() if controller1 else {},
        controller2.get_controller_inputs() if controller2 else {}
    )

async def process_triggers_and_buttons(controller1_state, controller2_state, controller1_state_old, controller2_state_old, gamepad, shift_active, button_mappings, shift_button_mappings):
    mappings_set = shift_button_mappings if shift_active else button_mappings

    for side, state, state_old in [("controller1", controller1_state, controller1_state_old), ("controller2", controller2_state, controller2_state_old)]:
        for input_name, conf in mappings_set.get(side, {}).items():
            ...
            elif input_name.startswith("ulButtonPressed_"):
            ...
            elif input_name == "trackpad_pressed":
                if (pressed := state.get("trackpad_pressed", False)) != state_old.get("trackpad_pressed", False):
...
async def process_left_joystick(controller1_state, controller2_state, shift_active, gamepad, config):
    lx = apply_deadzone_axis(
        extract_input_value(controller1_state, controller2_state, config.get("LEFT_X_REMAP", "controller1:trackpad_x")),
        config.get("LEFT_X_DEADZONE", 0.1)
    ) if config.get("LEFT_X_ENABLED", True) else 0.0

    ly = apply_deadzone_axis(
        extract_input_value(controller1_state, controller2_state, config.get("LEFT_Y_REMAP", "controller1:trackpad_y")),
