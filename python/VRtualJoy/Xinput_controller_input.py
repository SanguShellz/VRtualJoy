...
def extract_input_value(controller1_state, controller2_state, remap_key):
    if ':' in remap_key:
        ...
        if controller == "controller1":
            return controller1_state.get(input_key, 0.0)
        elif controller == "controller2":
            return controller2_state.get(input_key, 0.0)
        return 0.0
...
async def process_left_joystick(controller1_state, controller2_state, left_grip, gamepad, config):
    l_joystick_x = apply_deadzone_axis(
        extract_input_value(controller1_state, controller2_state, config["LEFT_X_REMAP"]), config["LEFT_X_DEADZONE"]
    ) if config["LEFT_X_ENABLED"] else 0.0

    l_joystick_y = apply_deadzone_axis(
        extract_input_value(controller1_state, controller2_state, config["LEFT_Y_REMAP"]), config["LEFT_Y_DEADZONE"]
...
async def process_triggers_and_buttons(controller1_state, controller2_state, controller1_state_old, controller2_state_old, gamepad, shift_active, config):
    mappings_set = config["SHIFT_BUTTON_MAPPINGS"] if shift_active else config["BUTTON_MAPPINGS"]

    for side, state, state_old in [
        ("controller1", controller1_state, controller1_state_old),
        ("controller2", controller2_state, controller2_state_old)
    ]:
        ...
        for input_name, conf in mappings.items():
            ...
            # Trackpad click
                ...
                was_pressed = state_old.get("trackpad_pressed", False)
                is_pressed = state.get("trackpad_pressed", False)
                if is_pressed != was_pressed and isinstance(button, int):
                    if is_pressed:
                        gamepad.press_button(button=button)
                    else:
