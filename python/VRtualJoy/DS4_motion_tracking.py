...
def initialize_vr_devices():
    ...
    controller1, controller2, hmd = None, None, None
    ...
    for dev in v.devices.values():
        if dev.device_class.lower() == "controller":
            ...
            if role == "left" and controller1 is None:
                controller1 = dev
            elif role == "right" and controller2 is None:
                controller2 = dev
...
async def apply_headtracking_to_right_stick(hmd, controller1_state, controller2_state, gamepad, yaw_smoother, pitch_smoother, config):
    raw_r_x = remap_float_axis(controller2_state.get("trackpad_x", 0.0) if controller2_state else 0.0)
    raw_r_y = remap_float_axis(controller2_state.get("trackpad_y", 0.0) if controller2_state else 0.0)

    processed_r_x = apply_sensitivity(apply_deadzone(raw_r_x, config.get("RX_DEADZONE", 0.1)), config.get("HEADTRACKING_SENSITIVITY_YAW", 1.5))
    processed_r_y = apply_sensitivity(apply_deadzone(raw_r_y, config.get("RY_DEADZONE", 0.1)), config.get("HEADTRACKING_SENSITIVITY_PITCH", 1.5))
    ...
    if config.get("HEADTRACKING_ENABLED", True) and hmd:
        ...
        if pose:
            ...
            if config.get("HEADTRACKING_YAW_ENABLED", True):
                yaw = apply_sensitivity(apply_deadzone(raw_yaw, config.get("HEADTRACKING_DEADZONE_X", 0.1)), config.get("HEADTRACKING_SENSITIVITY_YAW", 1.5))
            ...
            if config.get("HEADTRACKING_PITCH_ENABLED", True):
                pitch = apply_sensitivity(apply_deadzone(raw_pitch, config.get("HEADTRACKING_DEADZONE_Y", 0.1)), config.get("HEADTRACKING_SENSITIVITY_PITCH", 1.5))
